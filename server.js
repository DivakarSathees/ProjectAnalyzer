...
async function TestCodeSplitter(element, keyString, testCodeData) {
  ...
  const url = `https://sonarcloud.io/api/sources/lines?key=iamneo-production-2_${keyString}%3A${element.path}&from=1&to=502`;
...
async function DirHandler(element, keyString, testCodeData) {
  ...
  const dirUrl = `https://sonarcloud.io/api/measures/component_tree?ps=100&s=qualifier%2Cname&component=iamneo-production-2_${keyString}%3A${element.path}&metricKeys=ncloc%2Cvulnerabilities%2Cbugs%2Ccode_smells%2Csecurity_hotspots%2Ccoverage%2Cduplicated_lines_density&strategy=children`;
...
// });
  ...
  //   `https://sonarcloud.io/api/project_branches/list?project=iamneo-production-2_${key}`);

    // const sonarAddedDate = await axios.get(`https://sonarcloud.io/api/project_branches/list?project=iamneo-production-2_${key}`, {
...
const getCode = async (keyString, testCodeData) => {
  try {
    ...
    const files = `https://sonarcloud.io/api/measures/component_tree?ps=100&s=qualifier%2Cname&component=iamneo-production-2_${keyString}&metricKeys=ncloc%2Cvulnerabilities%2Cbugs%2Ccode_smells%2Csecurity_hotspots%2Ccoverage%2Cduplicated_lines_density&strategy=children`;
